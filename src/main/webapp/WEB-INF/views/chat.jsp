	...
	$(document).ready(function() {
			...
			$.ajax({
				...
				success: function(result) {
					...
					if(result.length > 1) {
						...
						console.log("${sessionScope.company.companyName}");
						
						for(i = 0; i < result.length; i++) {
							
							if(result[i].companyName == "${sessionScope.company.companyName}" || result[i].customerNnm == "${sessionScope.customer.customerNnm}") {
								addRoom($(".chat-list"), result[i].companyName, result[i].customerNnm, result[i].customerGender, result[i].roomId, true);
							} 
		...
		function addRoom(target, company_name, customer_nnm, customer_gender, room_id, position) {
			let html = "<li class='chat-room-list' value='" + room_id + "'>";
				...
				if('${sessionScope.customer.customerNnm}' == customer_nnm) {
					html += "<span class='chat-nnm'>" + company_name + "</span>";					
				} else if('${sessionScope.company.companyName}' == company_name) {
